...
const _RouteSearch = ({ className = '' }: RouteSearchProps): JSX.Element => {
    ...
    const debouncedSearch = useCallback(
        debounce((term: string) => {
            ...
            try {
                ...
                const _results = routes.filter((route) =>
    ...
    // Handle input change
    const _handleInputChange = (_e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const _handleRouteSelect = (_routeId: number) => {
        const route = searchResults.find(_r => _r.id === _routeId);
        if (route) {
            // Generate a slug from route.from and route.to
            const _slug = `${route.from.toLowerCase()}-to-${route.to.toLowerCase()}`.replace(/\s+/g, '-');
            _router.push(`/oneway/${_slug}`);
        }
        setShowResults(false);
        setSearchTerm('');
    };
        ...
        <div className={`relative ${className}`}>
            ...
            {showResults && searchResults.length > 0 && (
                <div className="absolute z-50 mt-1 w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md shadow-lg max-h-[300px] overflow-y-auto">
                    <div className="p-2">
                        {searchResults.map((route): JSX.Element => (
                            <button
                                key={route.id}
                                ...
                                onClick={() => _handleRouteSelect(route.id)}
                            >
