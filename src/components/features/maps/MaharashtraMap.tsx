...
export function MaharashtraMap(): JSX.Element {
    ...
    const _handlePinHover = (destination: Destination) => {
        setHoveredDestination(destination);
    };

    const _handlePinClick = (destination: Destination) => {
        setSelectedDestination(destination);
    };

    const _navigateToDestination = (slug: string) => {
        _router.push(`/maharashtra-tours/${slug}`);
    };
    ...
    return (
        <div className="relative w-full h-[500px] rounded-lg overflow-hidden border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900">
            ...
            <div className="relative w-full h-full">
                ...
                {_destinations.map((destination): JSX.Element => (
                    <button
                        key={destination.id}
                        className={`absolute transform -translate-x-1/2 -translate-y-1/2 group z-10 ${selectedDestination?.id === destination.id ? 'scale-125' : ''
                            }`}
                        style={{ left: `${destination.location.x}%`, top: `${destination.location.y}%` }}
                        onMouseEnter={() => _handlePinHover(destination)}
                        onMouseLeave={() => setHoveredDestination(null)}
                        onClick={() => _handlePinClick(destination)}
                    >
                        ...
                        <div className="flex-1">
                            ...
                            <div className="mt-3 flex gap-3">
                                <button
                                    ...
                                    onClick={() => _navigateToDestination(selectedDestination.slug)}
                                >
                                    Explore
                                </button>
                                <button
