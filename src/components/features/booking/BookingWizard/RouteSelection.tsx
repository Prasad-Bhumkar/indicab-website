...
export default function RouteSelection({
    ...
    updateFormData,
    setIsValid
}: RouteSelectionProps): JSX.Element {
    ...
    const getSuggestions = (input: string): string[] => {
        ...
        return _popularCities.filter(_city =>
            _city.toLowerCase().includes(_inputLower)
        );
    };

    // Handle pickup input change
    const _handlePickupChange = (_e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const _handleDropoffChange = (_e: React.ChangeEvent<HTMLInputElement>) => {
        const value = _e.target.value;
    ...
    const _selectPickupSuggestion = (suggestion: string) => {
    ...
    const _selectDropoffSuggestion = (suggestion: string) => {
    ...
    }, [formData.pickup, formData.dropoff, validateForm]);
            ...
            <div className="grid gap-6">
                    ...
                    <div className="flex-1 relative">
                        ...
                        <div className="relative">
                            ...
                            {showDropoffSuggestions && dropoffSuggestions.length > 0 && (
                                <div className="absolute z-10 mt-1 w-full bg-white dark:bg-gray-800 shadow-lg rounded-md border border-gray-200 dark:border-gray-700 max-h-60 overflow-auto">
                                    {dropoffSuggestions.map((suggestion): JSX.Element => (
                                        <div
                                            key={suggestion}
                                            className="px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer text-gray-800 dark:text-gray-200"
                                            onMouseDown={() => _selectDropoffSuggestion(suggestion)}
                                        >
                                            {suggestion}
                                        </div>
                                    ))}
                ...
                <div className="mt-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {popularRoutes.map((route): JSX.Element => (
                            <div
