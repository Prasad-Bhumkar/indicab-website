...
test.describe('Booking Flow', () => {
    test('complete booking process from homepage', async ({ page }) => {
        ...
        const _fromField = page.getByLabel(/from/i);
        await expect(_fromField).toHaveValue('Delhi');

        const _toField = page.getByLabel(/to/i);
        await expect(_toField).toHaveValue('Agra');
        ...
        if (await dateInput.isVisible()) {
            ...
            const _formattedDate = tomorrow.toISOString().split('T')[0]; // YYYY-MM-DD
    ...
    test('view and filter routes on routes page', async ({ page }) => {
        ...
        const _fromField = page.getByLabel(/from/i);
        await expect(_fromField).toHaveValue('Mumbai');

        const _toField = page.getByLabel(/to/i);
        await expect(_toField).toHaveValue('Pune');
    });
});
