...
export async function connectDB(): Promise<typeof mongoose> {
    if (global.indicabMongooseCache.conn) {
        ...
        try {
            ...
            _logger.warn('MongoDB connection stale, reconnecting...', { error });
    ...
    if (!global.indicabMongooseCache.promise) {
        ...
        global.indicabMongooseCache.promise = mongoose.connect(MONGODB_URI as string, _opts)
            .then(conn => {
    ...
    try {
        ...
        _logger.debug('Using existing MongoDB connection');
    } catch (error) {
        _logger.error('MongoDB connection failed:', { error });
