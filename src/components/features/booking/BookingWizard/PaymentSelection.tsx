...
export default function PaymentSelection({
	...
	const calculateFareBreakdown = useCallback((): { basePrice: number; tax: number; discount: number; totalPrice: number } => {
		const basePrice = formData.basePrice || 0;
		const tax = formData.tax || Math.round(basePrice * 0.05);
		const discount = formData.discount || 0;
		const totalPrice =
			formData.totalPrice || Math.round(basePrice + tax - discount);
	...
	const _handlePaymentMethodChange = (method: "card" | "upi" | "cash"): void => {
		updateFormData({ paymentMethod: method });
	};

	// Apply promo code
	const _handlePromoCodeChange = (_e: React.ChangeEvent<HTMLInputElement>): void => {
		updateFormData({ promoCode: _e.target.value });
	};

	const _applyPromoCode = (): void => {
		...
		<div className="space-y-6">
			...
			<div className="grid gap-6">
				...
				<div className="grid gap-3">
					{_paymentMethods.map((method): JSX.Element => {
						const _Icon = method.icon;
						return (
							<button
								key={method.id}
								type="button"
								className={`flex items-center border rounded-lg p-4 cursor-pointer transition-colors w-full ${
				...
				{formData.paymentMethod === "card" && (
					<div className="p-4 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
						...
						<p className="text-xs text-gray-400 dark:text-gray-500">
							We support all major cards including Visa, Mastercard, RuPay, and
							American Express
						</p>
					</div>
				)}
					...
					<div className="flex space-x-2">
						<div className="relative flex-grow">
							...
							<input
								...
								onChange={_handlePromoCodeChange}
				...
				<div className="text-sm text-gray-500 dark:text-gray-400 space-y-2">
					<p>
						• Toll charges, if any, are not included and will be paid by the
						passenger directly
					</p>
					<p>
						• If your trip duration exceeds the estimate, additional charges may
						apply
					</p>
					<p>
						• A 5% cancellation fee applies for cancellations within 1 hour of
						the pickup time
					</p>
				</div>
			</div>
