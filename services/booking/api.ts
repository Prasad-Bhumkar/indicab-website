...
export const getVehicles = async (): Promise<VehicleType[]> => {
    ...
    try {
        ...
        _logger.error('Vehicle API Error:', error);
        throw error;
    }
};

export const submitBooking = async (_booking: BookingFormData): Promise<{ success: boolean }> => {
    try {
        ...
        _logger.error('Booking API Error:', error);
        throw error;
    }
};
