...
export default function TourPackageCard({ package: pkg }: TourPackageCardProps): JSX.Element {
    ...
    return (
        ...
        >
            ...
            <div className="space-y-1 mb-6">
                {pkg.attractions.map((_attraction: string, _index: number): JSX.Element => (
                    <div key={_index} className="flex items-start">
                        <CheckCircle className="h-4 w-4 text-green-600 mr-2 mt-0.5 flex-shrink-0" />
                        <span className="text-gray-700 text-sm">{_attraction}</span>
                    </div>
                ))}
            </div>
        </PackageCard>
