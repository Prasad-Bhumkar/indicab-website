...
export const toBeValidRouteFormat: CustomMatcher = function (received) {
  ...
  const routeRegex = /^[A-Za-z\s]+ to [A-Za-z\s]+$/; // "City1 to City2" format
  const pass = typeof route === 'string' && routeRegex.test(route);
...
export const toBeValidFareFormat: CustomMatcher = function (received) {
  ...
  const priceRegex = /^₹\d{1,3}(,\d{3})*(\.\d{2})?$/; // ₹X,XXX or ₹X,XXX.XX format
  const pass = typeof price === 'string' && priceRegex.test(price);
